import { BaseActions } from "@/actions/base-actions";
import { AuthApi } from "@/clients/auth-api";
import { MailActions } from "@/actions/mail-actions";

export class DriverActions extends BaseActions {
    constructor(authApi: AuthApi = new AuthApi(), mailActions: MailActions = new MailActions()) {
        super(authApi, mailActions);
    }

    async register(email: string, password: string): Promise<void> {
        const reg = await this.auth.registerDriver(email, password);
        if (reg.status !== 201) {
            throw new Error(`Driver registration failed: ${reg.status}`);
        }
    }

    async registerVerify(email: string, password: string): Promise<void> {
        await this.register(email, password);
        const verifyUrl = await this.getVerifyURL(email);
        await this.verifyEmail(verifyUrl, "driver_confirm");
    }
}